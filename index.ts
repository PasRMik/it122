...
import { getAll, getItem, addOrUpdateItem, deleteItem } from "./data";
...
app.get("/detail", async (req: Request, res: Response) => {
    const game = await getItem(req.query.title as string);
    if (game) {
...
app.get("/api/games/:title", async (req: Request, res: Response) => {
...
app.delete("/api/games/:title", async (req: Request, res: Response) => {
